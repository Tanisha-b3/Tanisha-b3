<div align="center">

# TANISHA BORANA

### Software Developer • Full-Stack • AI & Backend

**Building practical products with React, Node.js, APIs, databases, and AI.**

<br />

<a href="https://www.linkedin.com/in/tanisha-borana-552797233">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<a href="mailto:tanishaborana970@gmail.com">
  <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
<a href="https://github.com/Tanisha-b3">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="https://leetcode.com/u/tanishaborana970/">
  <img src="https://img.shields.io/badge/LeetCode-1450-FFA116?style=for-the-badge&logo=leetcode&logoColor=white" />
</a>

</div>

---

## 👋 About

I'm a Software Developer focused on building **full-stack applications, backend APIs, and AI-enabled products**.

My work spans the complete development cycle — from responsive React interfaces and REST APIs to database design, third-party integrations, authentication, deployment, and cloud tooling.

```text
Frontend        → React • TypeScript • JavaScript
Backend         → Node.js • Express.js • REST APIs
Data            → MongoDB • MySQL • Firebase
AI              → OpenAI • Gemini • Vapi • Deepgram • ElevenLabs
Cloud / DevOps  → AWS • Azure • Docker • Terraform • Vercel
Tools            → Git • GitHub • Postman • VS Code
```

---

## ⚡ What I Work On

| Area | Focus |
|---|---|
| 🖥️ **Full-Stack Development** | React applications, Node.js services, REST APIs |
| 🤖 **AI Engineering** | AI integrations, voice workflows, personalized recommendations |
| 🔌 **Backend Engineering** | API design, authentication, business logic, data workflows |
| 🗄️ **Databases** | MongoDB, MySQL, Firebase |
| ☁️ **Cloud & DevOps** | AWS, Azure, Docker, Terraform, Vercel |
| 🧩 **Product Development** | Turning requirements into usable end-to-end features |

---

# 💼 Experience

## Full Stack Developer — Autoniv AI

**Jun 2026 – Present · Remote**

- Developed **React, TypeScript, and Node.js** modules for a multi-tenant SaaS platform supporting AI voice agents and chatbots.
- Owned **6+ AI-agent workflows** covering creation, voice configuration, knowledge setup, testing, deployment, and monitoring.
- Integrated **8+ external services**, including Vapi, Deepgram, OpenAI, ElevenLabs, WhatsApp, and Twilio.
- Designed and engineered **23 Express.js REST API modules** covering agent configuration, subscriptions, billing, usage tracking, and platform operations.
- Worked across frontend and backend workflows to turn product requirements into integrated application features.

## Software Developer — Apaxion Technology

**Use the verified date range shown on your resume / LinkedIn**

- Partnered with a **4-member engineering team** to build QRCrave, a QR-based restaurant ordering platform.
- Built **3 customer workflows** covering QR scanning, digital menu browsing, and checkout.
- Connected React interfaces with Node.js APIs across **3 ordering views** supporting menu and order management.
- Integrated frontend components with backend APIs to deliver connected ordering functionality.

---

# 🚀 Featured Projects

## 🍽️ QRCrave

**QR-based restaurant ordering platform**

`React.js` `Node.js` `Express.js` `MongoDB` `REST APIs`

A digital restaurant ordering experience centered around QR-based menu access, customer ordering, and order-management workflows.

**Core work**
- QR-based menu access
- Digital menu browsing
- Customer ordering
- Cart and checkout workflows
- REST API integration
- Responsive React interface

**[Live Website](https://qrcrave.com/)**

---

## 📅 Event Management System

**Full-stack event management platform**

`React` `Node.js` `MongoDB` `Socket.io` `JWT` `Docker` `Terraform` `Twilio`

A full-stack platform covering event creation, discovery, dashboards, ticketing, authentication, and real-time engagement.

**Core work**
- JWT authentication
- Event creation and management
- Event discovery
- Dashboards
- Ticketing workflows
- Real-time discussion boards
- Live polls
- Real-time Q&A
- Twilio notifications
- Docker containerization
- Terraform infrastructure

**[Live Demo](https://event-management-olive-beta.vercel.app/) · [Source Code](https://github.com/Tanisha-b3/Event_management)**

---

## ✈️ ExploraTrails

**AI-powered travel planning application**

`React.js` `Firebase` `Gemini API`

A personalized travel planner that generates travel recommendations based on user location, available days, and budget. The public repository documents Gemini API integration, Firebase-backed data, and a responsive React interface. 

**Core work**
- Personalized travel plans
- Gemini API integration
- Location-specific recommendations
- Budget-aware planning
- Firebase authentication/data
- Responsive interface

**[Live Demo](https://travelappproject.vercel.app/) · [Source Code](https://github.com/Tanisha-b3/Travel_app)**

---

# 🧰 Tech Stack

### Languages

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)
![REST](https://img.shields.io/badge/REST_APIs-02569B?style=flat-square)

### Databases

![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black)

### AI & Integrations

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini_API-4285F4?style=flat-square&logo=google&logoColor=white)
![Vapi](https://img.shields.io/badge/Vapi-111827?style=flat-square)
![Deepgram](https://img.shields.io/badge/Deepgram-101828?style=flat-square)
![ElevenLabs](https://img.shields.io/badge/ElevenLabs-000000?style=flat-square)
![Twilio](https://img.shields.io/badge/Twilio-F22F46?style=flat-square&logo=twilio&logoColor=white)

### Cloud & DevOps

![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-844FBA?style=flat-square&logo=terraform&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white)

---

# 🏆 Achievements

| Achievement | Result |
|---|---|
| 🧩 **LeetCode** | 1450 Rating · 450+ Problems |
| ⭐ **HackerRank** | 3-Star Java |
| 🏆 **Smart India Hackathon** | 2023 |
| 🌐 **Hacktoberfest** | 2023 Contributor |

---

# 🎓 Education

**Lakshmi Narain College of Technology, Bhopal**

**B.Tech — Information Technology** · CGPA **8.4** · 2021–2025

---

# 📌 GitHub Highlights

<div align="center">

<a href="https://github.com/Tanisha-b3?tab=repositories">
<img src="https://img.shields.io/badge/45%2B_Public_Repositories-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://github.com/Tanisha-b3?tab=stars">
<img src="https://img.shields.io/badge/GitHub_Stars-7-F5C518?style=for-the-badge&logo=github&logoColor=181717" />
</a>

</div>

<br />

I use GitHub to document projects, experiment with technologies, and build practical software across frontend, backend, AI, and cloud development.

**[Explore all repositories →](https://github.com/Tanisha-b3?tab=repositories)**

---

# 🌱 Currently Exploring

```text
AI Applications
      ↓
Backend Engineering
      ↓
REST API Architecture
      ↓
Cloud & DevOps
      ↓
Scalable Full-Stack Systems
```

---

# 📫 Let's Connect

<div align="center">

<a href="https://www.linkedin.com/in/tanisha-borana-552797233">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:tanishaborana970@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

<a href="https://leetcode.com/u/tanishaborana970/">
<img src="https://img.shields.io/badge/LeetCode-Profile-FFA116?style=for-the-badge&logo=leetcode&logoColor=white" />
</a>

<a href="https://github.com/Tanisha-b3">
<img src="https://img.shields.io/badge/GitHub-Explore-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

</div>

# 📌 GitHub Highlights

<div align="center">

<a href="https://github.com/Tanisha-b3?tab=repositories">
<img src="https://img.shields.io/badge/45%2B_Public_Repositories-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://github.com/Tanisha-b3?tab=stars">
<img src="https://img.shields.io/badge/GitHub_Stars-7-F5C518?style=for-the-badge&logo=github&logoColor=181717" />
</a>

</div>

---

<div align="center">

### Build • Learn • Ship • Improve

</div>
